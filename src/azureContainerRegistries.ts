...
export class AzureContainerRegistries extends DockerTreeBase<ACRTreeItem> implements vscode.TreeDataProvider<ACRTreeItem> {
    ...
    public getChildren(element?: ACRTreeItem): Thenable<ACRTreeItem[]> {
        if (element === undefined) {
            ...
            try {
                regs = JSON.parse(/*Executor.execSync(`docker exec azure-cli az acr list`)*/ "xxxx");
            } catch (error) {
            ...
            const ret = [];
            const images = JSON.parse(/*Executor.execSync(`docker exec azure-cli az acr repository list -n ${element.node.name}`)*/ "xxxxx")
            ...
            const ret = [];
            const tags = JSON.parse(/*Executor.execSync(`docker exec azure-cli az acr repository show-tags --name ${element.node.parent.name} --repository ${element.node.name}`)*/ "XXXXX")
